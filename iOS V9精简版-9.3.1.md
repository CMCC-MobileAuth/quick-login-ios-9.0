...
3. 一键登录服务目前支持中国移动2/3/4G/5G（2,3G因为无线网络环境问题，时延和成功率会比4G低）
4. 该SDK版本仅支持中国移动号码用户取号
...
> **小技巧：**如果开发者希望能够单独的给**预取号**和**授权登录**请求设置超时时间，可以在调用这俩方法前都分别设置一次这个超时时间即可。
...
本方法用于发起取号请求，SDK完成网络判断、蜂窝数据网络切换等操作并缓存凭证scrip。<font color="red">业务在调用预取号和授权方法前，请务必自行判断清楚当前用户的网络类型和有无SIM卡等情况。</font>

**请求示例代码**

```objective-c
[UASDKLogin.shareLogin getPhoneNumberWithNetworkType:@"1" completion:^(NSDictionary * _Nonnull sender) {
    NSLog(@"result = %@", sender);
}];
```
...
```objective-c
- (void)getPhoneNumberWithNetworkType:(NSString *_Nonnull)networkType 
...
| 参数        | 类型     | 说明                                                  |
| ----------- | -------- | ----------------------------------------------------- |
| networkType | NSString | 0.无网络;</br>1.数据流量;</br>2.wifi;</br>3.wifi+数据 |
...
| operatorType  | NSString | 运营商类型：固定值是1（中国移动） |
| traceId       | NSString | 用于定位SDK问题 |

> 注意1：若手机实际情况为无网络，但app仍然调用取号方法，SDK会报200050错误码
>
> 注意2：若手机实际情况为蜂窝未开启，但已连接wifi，取号会直接报200096错误码（当前网络不支持取号）
>
> 注意3：手机实际情况为无SIM，并且未连接wifi，取号会直接报200050错误码；但已连接wifi，取号会直接报200096错误码
>
...
-(void)authorizeLoginButtonClick{
    ...
    [UASDKLogin.shareLogin getAuthorizationWithNetworkType:@"1" completion:^(NSDictionary * _Nonnull sender) {
        if ([sender[@"resultCode"] isEqualToString:@"103000"]) {
            NSLog(@"授权登录成功:%@",sender);
        } else {
            NSLog(@"授权登录失败:%@",sender);
...
```objective-c
- (void)getAuthorizationWithNetworkType:(NSString *_Nonnull)networkType    
                             completion:(void (^)(NSDictionary *sender))completion;
```
...
```objective-c
- (void)mobileAuthWithNetworkType:(NSString *_Nonnull)networkType
                       completion:(void (^)(NSDictionary *sender))completion;
```
...
本方法用于删除取号方法`getPhoneNumberWithNetworkType:completion:`成功后返回的取号凭证scrip
